...
function groupDifferences(allocations, width) {
  ...
  // Corresponds to: let $gdiffs := $allocations/allocation/(floor((siteIMDDecile - allocationIMDDecile) div $width))
  const gdiffs = allocations
    .filter(alloc => typeof alloc.siteIMDDecile === 'number' && typeof alloc.allocationIMDDecile === 'number')
    .map(alloc => {
      const diff = alloc.siteIMDDecile - alloc.allocationIMDDecile;
 ...
 * @param {number} nBoot - Number of bootstrap samples (default 1000)
 * @returns {Object} Object with lower and upper bounds
 */
function bootstrapMedianCI(data, confidenceLevel = 0.95, nBoot = 1000) {
  ...
  const lowerIndex = Math.floor(lowerPercentile / 100 * nBoot);
  const upperIndex = Math.floor(upperPercentile / 100 * nBoot);

  return {
    lower: medians[lowerIndex],
    upper: medians[upperIndex]
  };
}

export { groupDifferences, scatter_sums, correlation, scatter_diffs, bootstrapMedianCI };
