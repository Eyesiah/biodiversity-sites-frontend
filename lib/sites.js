      ...
      for (const [baselineType, baselineAmount] of sortedBaseline) {
        ...
        if (enhancementType == 'Enhanced') {
          if (baselineHabType == improvementHabType) {
            ...
            // different habitat: only allowed if same broad habitat
            const sameBroadHab = baselineBroadHabType == improvementBroadHabType;
            if (!sameBroadHab) {
              continue;
            }
            if (!distinctivenessImproves) {
              // if distinctiveness doesn't improve, must be the same distinctiveness and improved condition
              if (improvementDistinctiveness != baselineDistinctiveness) {
                continue;
              }
              if (getConditionScore(improvementCond) <= getConditionScore(baselineCond)) {
                continue;
        ...
        // must also be an HU gain, unless its a creation with improved distinctiveness
        if (!distinctivenessImproves || enhancementType == 'Enhanced') {
