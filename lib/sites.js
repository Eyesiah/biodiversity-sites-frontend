...
export function processSiteForListView(site) {
  ...
  const improvementHUs = [...site.improvements.areas, ...site.improvements.hedgerows, ...site.improvements.watercourses, ...site.improvements.trees].reduce((acc, h) => acc + h.HUs, 0);
  const huGain = [...site.improvements.areas, ...site.improvements.hedgerows, ...site.improvements.watercourses, ...site.improvements.trees].reduce((acc, h) => acc + (h.HUs - h.baselineHUs), 0);
  return {
    ...
    huGain: huGain,
    huGainPerHa: huGain / (site.siteSize || 1)
  }
}
  ...
  allSites.forEach(site => {
    ...
    if (site.allocations) {
      ...
      site.allocations.forEach(alloc => {
        totalAllocationHUs += (alloc.sizeUnits || 0);
...
export const getHabitatAssignmentData = (site, unit) => {
  ...
  allocateRemaining((baseline, improvement, baselineSize, improvementSize) => {
    return Math.abs(baselineSize - improvementSize) < 0.01;
  });
