...
export const isIndividualTree = (habitat) => getHabitatGroup(habitat.type) == 'Individual trees';
    ...
    if (site.improvements) {
      const processImprovementHabitats = (habitats, isArea) => {
        ...
        habitats.forEach(h => {
          ...
          if (type) {
            totalCreatedHUs += calculateImprovementHU(h.size, type, h.condition || 'N/A - Other', h.interventionType || '').HUs;
