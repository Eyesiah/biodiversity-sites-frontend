import { getHabitatDistinctiveness, calculateImprovementHU, getHabitatGroup, getDistinctivenessScore, getConditionScore } from '@/lib/habitat';
...
export function processSiteForListView(site) {
  ...
  return {
    ...
    name: site.name,
    bgsWebsite: site.bgsWebsite || null,
    bgsReferenceUrl: site.bgsReferenceUrl || null,
    miscUrls: site.miscUrls || null,
    ...
    huGain: huGain,
    publishedDate: site.publishedDate ?? null,
    publishedDatePreTracking: site.publishedDatePreTracking ?? false,
  }
}
...
export function processSiteDataForIndex(allSites) {
  ...
  allSites.forEach(site => {
    if (site.habitats) {
      ...
      const siteBaselineTreesSize = site.habitats.trees.reduce((acc, hab) => acc + hab.size, 0);
      baselineTreesSize += siteBaselineTreesSize;
      baselineAreaSize += site.habitats.areas.reduce((acc, hab) => acc + hab.size, 0) + siteBaselineTreesSize;
    ...
    if (site.improvements) {
      ...
      const siteImprovementsTreesSize = site.improvements.trees.reduce((acc, hab) => acc + hab.size, 0);
      improvementsTreesSize += siteImprovementsTreesSize;
      improvementsAreaSize += site.improvements.areas.reduce((acc, hab) => acc + hab.size, 0) + siteImprovementsTreesSize;
    ...
    if (site.allocations) {
      ...
      site.allocations.forEach(alloc => {
        ...
        if (alloc.habitats) {
          ...
          allocatedTreesSize += (alloc.habitats.trees || []).reduce((acc, hab) => acc + hab.size, 0);
        }
      });
    }
  });
...
export const getHabitatAssignmentData = (site, unit) => {
  ...
  if (baselineHabitats) {
    for (const habitat of baselineHabitats) {
      if (habitat.size > 0 && getHabitatDistinctiveness(habitat.type) != null) {
  ...
  const habitatSorter = (isBaseline) => {
    return ([keyA, remainingSizeA], [keyB, remainingSizeB]) => {
      ...
      if (scoreA == scoreB) {
        // sort 'assesment N/A' below 'Low', they normally have the same score of 1
        const condScoreA = conditionA.toLowerCase().trim() == 'condition assessment n/a' ? 0.5 : getConditionScore(conditionA);
        const condScoreB = conditionB.toLowerCase().trim() == 'condition assessment n/a' ? 0.5 : getConditionScore(conditionB);
        if (condScoreA == condScoreB) {
          ...
          if (isBaseline) {
            ...
            // when sorting improvements, sort by smallest first
            return remainingSizeB - remainingSizeA;
          }
        } else {
          return isBaseline ? condScoreA - condScoreB : condScoreB - condScoreA;
        }
      } else {
        return isBaseline ? scoreA - scoreB : scoreB - scoreA;
      }
    }
  }
  ...
  const allocateRemaining = (isConversionPossiblePredicate) => {
    ...
    // Sort remaining improvements and iterate over them.
    // Start with the ones with the highest score, as they will only be compatible with sufficiently low quality baselines
    const sortedImprovements = Array.from(remainingImprovement.entries())
      .filter(([, amount]) => amount > 0)
      .sort(habitatSorter(false));

    for (const [improvementType, improvementAmount] of sortedImprovements) {
      if (improvementAmount <= 0.0001) {
        continue;
      }
      ...
      let remainingToAllocate = improvementAmount;

      // sort the baselines again and iterate over them, trying to allocate to this improvement
      // start with the ones with the lowest score, so that we are more likely to retain better baselines
      const sortedBaseline = Array.from(remainingBaseline.entries())
        .sort(habitatSorter(true));

      for (const [baselineType, baselineAmount] of sortedBaseline) {
        if (baselineAmount <= 0.0001 || !isConversionPossiblePredicate(baselineType, improvementType, baselineAmount, remainingToAllocate)) continue;

        const allocatedAmount = Math.min(remainingToAllocate, baselineAmount);
        if (allocatedAmount > 0) {
          ...
          if (enhancementType == 'Enhanced') {
            // enhancements require an improvement in HUs (note the trading rules will also be checked here)
            ...
            // creation requires the distinctiveness is at least as high
            const baselineDistinctiveness = getDistinctivenessScore(baselineHabType);
            if (baselineDistinctiveness > improvementDistinctiveness) {
              continue;
            }
            // if the distinctiveness is the same, the condition must be improved (or the same if a different hab)
            if (baselineDistinctiveness == improvementDistinctiveness) {
              const improvementCondScore = getConditionScore(improvementCond);
              const baselineCondScore = getConditionScore(baselineCond);
              if (baselineHabType == improvementHabType) {
                // same habitat: condition must be improved
                if (improvementCondScore <= baselineCondScore) {
                  continue;
                }
              } else {
                // habitat change: condition must be improved or the same
                if (improvementCondScore < baselineCondScore) {
                  continue;
                }
              }
            }
            // and for area habitats, that the broad habitat type changes
            if (unit == 'areas') {
  ...
  // 1a: allocate enhanced habitats only (same habitat type and higher condition score)
  allocationLogger(`ALLOCATION 1a: Enhanced, same type`);
  allocateRemaining((baseline, improvement) => {

    const isEnhancement = improvement.includes('|') && improvement.split('|')[2] == 'Enhanced';
    if (!isEnhancement) {
      return false;
    }
  ...
  // 1b: allocate enhanced habitats within same group
  allocationLogger(`ALLOCATION 1b: Enhanced, same broad habitat`);
  allocateRemaining((baseline, improvement) => {
    const isEnhancement = improvement.includes('|') && improvement.split('|')[2] == 'Enhanced';
    if (!isEnhancement) {
      return false;
...
export const convertSankeySourceDataToGraph = (unitName, unitData) => {
  const nodes = [];
  const nodeLabels = [];
  const nodeColors = [];
  const links = [];
  ...
  for (const [linkKey, linkValue] of unitData.aggregatedLinks.entries()) {
    ...
    if (sourceIndex !== undefined && targetIndex !== undefined) {
      ...
      const baseHabitatName = sourceType.includes('|') ? sourceType.split('|')[0] : sourceType;
      const baseConditionName = sourceType.includes('|') ? sourceType.split('|')[1] : '';
      const improvementHabitatName = improvementType.includes('|') ? improvementType.split('|')[0] : improvementType;
      const improvementConditionName = improvementType.includes('|') ? improvementType.split('|')[1] : '';
      const huData = enhancementType != 'Retained' ? calculateImprovementHU(linkValue, improvementHabitatName, improvementConditionName, enhancementType, baseHabitatName, baseConditionName) : {};
