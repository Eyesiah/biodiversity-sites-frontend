...
export function processSiteDataForIndex(allSites) {
  ...
  allSites.forEach(site => {
    if (site.habitats) {
      ...
      baselineTreesSize += site.habitats.trees.reduce((acc, hab) => acc + hab.size, 0);
      baselineAreaSize += baselineTreesSize; // Keep trees in total area too
    ...
    if (site.improvements) {
      ...
      improvementsTreesSize += site.improvements.trees.reduce((acc, hab) => acc + hab.size, 0);
      improvementsAreaSize += improvementsTreesSize; // Keep trees in total area too
