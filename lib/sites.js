...
export function processSiteDataForIndex(allSites) {
  ...
  allSites.forEach(site => {
    ...
    if (site.improvements) {
      ...
      // Calculate retained trees separately
      retainedTreesSize += site.improvements.trees.reduce((acc, hab) => {
        const subRows = hab.subRows || [];
        return acc + subRows.filter(sub => sub.interventionType === 'Retained').reduce((subAcc, sub) => subAcc + sub.size, 0);
      }, 0);
