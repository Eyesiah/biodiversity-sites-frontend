...
function loadTemporalMultipliers() {
  ...
  results.data.forEach(row => {
    const year = row['Year'];
    const multiplier = row['Time to target Multiplier'];
  ...
  if (effectiveTimeToTarget == "Not Possible") {
    ...
    if (!isNaN(timeToTargetYears)) {
      ...
      const multiplier = temporalMultipliers.get(String(timeToTargetYears)) ??
                        (timeToTargetYears > 31 ? temporalMultipliers.get("30+") : null);
  ...
  const collated = habitats.reduce((acc, habitat) => {
    ...
    if (!acc[key].subRows[subKey]) {
      ...
      acc[key].subRows[subKey] = {
        ...
        type: habitat.type,
        timetotarget: isImprovement && !isEnhancement ? getTimeToTarget(habitat.type, habitat.condition) : null,
        temporalMultiplier: isImprovement && !isEnhancement ? calcTemporalRisk(habitat.type, habitat.condition) : null,
        parcels: 0,
        area: 0,
        HUs: 0,
      };
