  ...
  const collated = habitats.reduce((acc, habitat) => {
    ...
    if (!acc[key].subRows[subKey]) {
      acc[key].subRows[subKey] = {
        ...
        type: habitat.type,
        timetotarget: isImprovement ? getTimeToTarget(habitat.type, habitat.condition) : null,
        temporalMultiplier: isImprovement ? calcTemporalRisk(habitat.type, habitat.condition) : null,
        parcels: 0,
        area: 0,
        HUs: 0,
      };
