...
function calcEnhancedTimeToTarget(baselineHabitat, baselineCondition, enhancedHabitat, enhancedCondition) {
  ...
  if (baselineCondition == 'N/A' && broadHabitat !== 'Watercourses') {
  ...
  if (baselineHabitat == enhancedHabitat) {
    ...
    // Find the column based on baseline hab's distinctiveness
    const baselineDistinctiveness = getHabitatDistinctiveness(baselineHabitat);
    const columnName = distinctivenessColumns[baselineDistinctiveness?.toLowerCase()];
    return enhancementRow[columnName];
  }
}
...
export function calculateImprovementHU(size, habitat, condition, improvementType, timeToTargetOffset = 0, strategicSignificanceScore = 1.0, spatialRisk = 1.0, baselineHabitat = null, baselineCondition = null) {
  if (typeof size !== 'number' || isNaN(size) ||
    typeof strategicSignificanceScore !== 'number' || isNaN(strategicSignificanceScore) ||
  ...
  if (improvementType.toLowerCase() == 'creation') {
    ...
    if (baselineHabitat == null || baselineCondition == null) { 
      console.error(`calculateImprovementHU: enhanced improvement with no baseline habitat or condition`);
      return {HUs: 0};
    }
    ...
    if (HUData.timeToTarget == null) {
      console.error(`calculateImprovementHU: failed to calc calcEnhancedTimeToTarget`);
      return {HUs: 0};
    }
    ...
    HUData.difficultyFactor = calcDifficultyFactor(habitat);
    HUData.spatialRisk = spatialRisk;
    HUData.HUs = calculateEnhancedHU(
      size,
      habitat,
      condition,
      HUData.temporalRisk,
      HUData.difficultyFactor,
      getDistinctivenessScore(baselineHabitat),
      getConditionScore(baselineCondition),
...
function calculateEnhancedHU(size, habitat, condition, temporalMultiplier, difficultyMultiplier, baselineDistinctiveness, baselineCondition, strategicSignificance = 1.0, spatialRisk = 1.0) {
  ...
  const finalHU = result * strategicSignificance * spatialRisk;
