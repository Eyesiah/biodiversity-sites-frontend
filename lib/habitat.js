...
function loadHedgerowEnhancementMultipliers() {
  if (!fs) return new Map();

  const csvPath = path.join(process.cwd(), 'data', 'Temporal-Enhancement-Multipliers-Hedgerows.csv');
  const csvData = fs.readFileSync(csvPath, 'utf8');
  ...
  results.data.forEach(row => {
    const baselineHabitat = row['baseline habitat']?.trim();

    if (baselineHabitat) {
      // Store the entire row for lookup
      enhancementMap.set(baselineHabitat.toLowerCase(), row);
    }
  });
...
const hedgerowEnhancementMultipliers = loadHedgerowEnhancementMultipliers();
...
function loadLPAData() {
  ...
  // Build the map directly — filter, shape, and index in a single pass
    return rawLpas
    .filter((lpa) => lpa.id && lpa.id.startsWith('E'))
    .reduce((map, lpa) => {
      map.set(slugify(normalizeBodyName(lpa.name)), {
        id: lpa.id,
        name: lpa.name,
        adjacents: lpa.adjacents || [],
        size: lpa.size / 10000,
        adjacentsCount: lpa.adjacents ? lpa.adjacents.length : 0,
        planningPortalUrls: lpa.planningPortalUrls || [],
      });
      return map;
    }, new Map());
}
...
function loadLNRSData() {
  ...
  const lnrsMap = new Map();
  rawLnrs.forEach(lnrs => lnrsMap.set(slugify(normalizeBodyName(lnrs.name)), lnrs));

  return lnrsMap;
}

const lnrsData = loadLNRSData();

export function getLNRSData() {
  return lnrsData;
}
...
export const calcSpatialRiskCategory = (alloc, site) => {
  ...
  const siteLNRS = slugify(normalizeBodyName(site.lnrsName));
  const allocLNRS = slugify(normalizeBodyName(alloc.lnrs));
  ...
  if (siteLPA && allocLPA) {
    const adjacentLPASlugs = new Set(
      (lpaData.get(siteLPA)?.adjacents || []).map(a => slugify(normalizeBodyName(a.name)))
    );
    if (adjacentLPASlugs.has(allocLPA)) {
      return { cat: 'Neighbouring', from: 'LPA' };
    }
  }
  if (siteLNRS && allocLNRS) {
    const adjacentLNRSSlugs = new Set(
      (lnrsData.get(siteLNRS)?.adjacents || []).map(a => slugify(normalizeBodyName(a.name)))
    );
    if (adjacentLNRSSlugs.has(allocLNRS)) {
      return { cat: 'Neighbouring', from: 'LNRS' };
    }
  }
...
// Store original habitat names from CSV for display purposes
const originalHabitatNames = new Map();
...
function loadHabitatData() {
  ...
  results.data.forEach(row => {
    ...
    if (habitatType && distinctiveness) {
      const distinctivenessScore = Number(row['Distinctiveness Score']);
      ...
      // Store the original habitat name for display
      originalHabitatNames.set(habitatType.trim().toLowerCase(), habitatType);
...
export function getTimeToTarget(habitat, condition) {
  ...
  return tryConvertToNumber(timeToTarget);
}

export function getEffectiveTimeToTarget(timeToTarget, timeToTargetOffset = 0) {
  let effectiveTimeToTarget = timeToTarget;

  if (timeToTargetOffset !== 0) {
    if (typeof timeToTarget === 'string' && !isNaN(Number(timeToTarget))) {
      // Apply offset to numeric time to target: Base TtoT + Offset = Net TtoT
      const baseYears = Number(timeToTarget);
      effectiveTimeToTarget = Math.max(0, baseYears + timeToTargetOffset);
    } else if (timeToTarget === "30+") {
      // For 30+, apply offset but don't go below 0
      effectiveTimeToTarget = Math.max(0, 30 + timeToTargetOffset);
    } else if (typeof timeToTarget === 'number') {
      // If timeToTarget is already a number, apply offset directly
      effectiveTimeToTarget = Math.max(0, timeToTarget + timeToTargetOffset);
    } else {
      // For other cases (Not Possible, null, etc.), don't apply offset
    }
  }
  return effectiveTimeToTarget;
}

function calcTemporalRisk(habitat, condition, timeToTarget, timeToTargetOffset = 0) {
  ...
  let effectiveTimeToTarget = getEffectiveTimeToTarget(timeToTarget, timeToTargetOffset);

  if (effectiveTimeToTarget == "Not Possible") {
    const multiplier = temporalMultipliers.get("Not Possible") || 0;
    return multiplier;
  }
    ...
    const multiplier = temporalMultipliers.get("30+") || 0;
    return multiplier;
  }
  else {
    const timeToTargetYears = Number(effectiveTimeToTarget);
    if (!isNaN(timeToTargetYears)) {
      // Try to get the multiplier for the exact year
      const exactYearKey = String(timeToTargetYears);
      const multiplier = temporalMultipliers.get(exactYearKey);

      if (multiplier !== null && multiplier !== undefined) {
        return multiplier;
      }
      else {
        // Check if we have a "31+" entry for years beyond 31
        if (timeToTargetYears > 31) {
          const fallbackMultiplier = temporalMultipliers.get("31+");
          if (fallbackMultiplier !== null && fallbackMultiplier !== undefined) {
            return fallbackMultiplier;
          }
        }
        // For years 32+ without 31+ entry, or any other case, use 30+
        const fallbackMultiplier = temporalMultipliers.get("30+");
        return fallbackMultiplier !== null && fallbackMultiplier !== undefined ? fallbackMultiplier : 0;
      }
    }
    else {
...
function calcEnhancedTimeToTarget(baselineHabitat, baselineCondition, enhancedHabitat, enhancedCondition) {
  ...
  if (broadHabitat === 'Hedgerow') {
    // hedgerows have special handling with a different datatable
    const enhancedRow = hedgerowEnhancementMultipliers.get(baselineHabitat.toLowerCase());
    if (enhancedRow == null) {
      return null;
    }
    if (baselineHabitat == enhancedHabitat) {
      // if habitat doesn't change, look up by condition
      // 'fairly X' just becomes 'X'
      let baselineConditionLower = baselineCondition.toLowerCase();
      let enhancedConditionLower = enhancedCondition.toLowerCase();

      if (baselineConditionLower == 'Fairly Poor') {
        baselineConditionLower = 'poor';
      }
      if (baselineConditionLower == 'Fairly Good') {
        baselineConditionLower = 'good';
      }
      if (enhancedConditionLower == 'Fairly Poor') {
        enhancedConditionLower = 'poor';
      }
      if (enhancedConditionLower == 'Fairly Good') {
        enhancedConditionLower = 'good';
      }
      const conditionKey = `${baselineConditionLower} - ${enhancedConditionLower}`;
      return tryConvertToNumber(enhancedRow[conditionKey]);
    } else {
      return tryConvertToNumber(enhancedRow[enhancedHabitat.toLowerCase()]);
    }
  }
  ...
  if (baselineCondition.toLowerCase().includes('n/a')) {
    ...
    return tryConvertToNumber(enhancedRow[columnName]);
...
export function getDifficultyCreationMultiplier(habitat) {
  return tryConvertToNumber(habitatMap.get(habitat.toLowerCase())?.difficultyCreationMultiplier ?? 1);
}

export function getEnhancementDifficultyFactor(habitat) {
  return tryConvertToNumber(habitatMap.get(habitat.toLowerCase())?.difficultyEnhancementMultiplier ?? 1);
}

/**
 * Get the trading rules for a habitat based on its distinctiveness
 * @param {string} habitat The habitat type
 * @returns {string} The trading rule based on distinctiveness score
 */
export function getTradingRules(habitat) {
  ...
  // High (score 6) - Same habitat only (no trading to higher distinctiveness)
  if (distinctivenessScore === 6) {
    return 'same habitat only';
  }
  // Medium (score 4) - Same broad habitat type OR higher distinctiveness
  if (distinctivenessScore === 4) {
    return 'same broad habitat or better';
  }
  // Low (score 2) - Same distinctiveness band or better
  if (distinctivenessScore === 2) {
    return 'same distinctiveness band or better';
  }
  // V.Low (score 1 or 0) - Can be replaced with Low or higher distinctiveness
  return 'low or better';
}
...
export function checkTradingRules(baselineHabitat, targetHabitat) {
  ...
  // Check based on specific trading rules by distinctiveness score
  if (baselineDistinctiveness >= 8) {
    // Very High distinctiveness (score 8+) - Same habitat required
    if (baselineHabitat.toLowerCase() !== targetHabitat.toLowerCase()) {
      return {
        allowed: false,
        reason: `Trading rules violated: ${baselineHabitat} is Very High distinctiveness and requires same habitat. Cannot enhance to ${targetHabitat}.`
      };
    }
    return { allowed: true, reason: 'Same habitat - trading rules satisfied' };
  }

  if (baselineDistinctiveness === 6) {
    // High distinctiveness (score 6) - Same habitat only (no trading to higher distinctiveness)
    if (baselineHabitat.toLowerCase() !== targetHabitat.toLowerCase()) {
      return {
        allowed: false,
        reason: `Trading rules violated: ${baselineHabitat} is High distinctiveness and requires same habitat only. Cannot enhance to ${targetHabitat}.`
      };
    }
    return { allowed: true, reason: 'Same habitat - trading rules satisfied' };
  }
  ...
  if (baselineDistinctiveness === 4) {
    // Medium distinctiveness (score 4) - Same broad habitat type OR higher distinctiveness
    if (baselineHabitat.toLowerCase() === targetHabitat.toLowerCase()) {
      return { allowed: true, reason: 'Same habitat - trading rules satisfied' };
    }
    
    // Must be same broad habitat type OR higher distinctiveness
    // If different group, must have higher distinctiveness
    if (baselineGroup !== targetGroup && targetDistinctiveness <= baselineDistinctiveness) {
      return {
        allowed: false,
        reason: `Trading rules violated: ${baselineHabitat} requires same broad habitat type or higher distinctiveness. Cannot enhance from ${baselineGroup} to ${targetGroup} with same or lower distinctiveness.`
      };
    }
    
    // If same group but lower distinctiveness, also fail
    if (baselineGroup === targetGroup && targetDistinctiveness < baselineDistinctiveness) {
      return {
        allowed: false,
        reason: `Trading rules violated: ${baselineHabitat} requires same broad habitat type or higher distinctiveness. Cannot enhance to lower distinctiveness ${targetHabitat}.`
      };
    }
  }

  if (baselineDistinctiveness === 2) {
    // Low distinctiveness (score 2) - Same distinctiveness band or better
    if (baselineHabitat.toLowerCase() === targetHabitat.toLowerCase()) {
      return { allowed: true, reason: 'Same habitat - trading rules satisfied' };
    }
  ...
  if (baselineDistinctiveness <= 1) {
    // Very Low distinctiveness (score 1 or 0) - Can be replaced with Low or higher distinctiveness
    if (baselineHabitat.toLowerCase() === targetHabitat.toLowerCase()) {
      // Same V.Low habitat should fail
      return {
        allowed: false,
        reason: `Trading rules violated: ${baselineHabitat} is Very Low distinctiveness and cannot be replaced with the same habitat type.`
      };
    }
    
    if (targetDistinctiveness < 2) {
      return {
        allowed: false,
        reason: `Trading rules violated: ${baselineHabitat} is Very Low distinctiveness and can only be replaced with Low or higher distinctiveness habitats.`
      };
    }
    
    // For Very Low distinctiveness, allow trading to any Low or higher distinctiveness habitat
    // regardless of group (this is the "can be replaced with Low or higher" rule)
    return { allowed: true, reason: 'Trading rules satisfied' };
  }

  // Check habitat type compatibility (Hedgerow, Watercourse, Area)
  // This check should come AFTER distinctiveness checks to allow valid trades
  // Only apply habitat group restrictions for certain distinctiveness levels

  // For other distinctiveness levels, apply habitat group compatibility
...
function calculateCreationHU(size, habitat, condition, timeToTarget, timeToTargetOffset, strategicSignificanceScore, spatialRisk) {
  const HUData = {};
  HUData.timeToTarget = timeToTarget;
  HUData.temporalRisk = calcTemporalRisk(habitat, condition, HUData.timeToTarget, timeToTargetOffset);
...
export function calculateImprovementHU(size, habitat, condition, improvementType, baselineHabitat = null, baselineCondition = null, timeToTargetOffset = 0, strategicSignificanceScore = 1.0, spatialRisk = 1.0) {
  ...
  if (improvementType.toLowerCase() == 'creation') {
    const HUData = calculateCreationHU(size, habitat, condition, getTimeToTarget(habitat, condition), timeToTargetOffset, strategicSignificanceScore, spatialRisk);
    ...
    // Check if baseline habitat has zero distinctiveness and target has higher distinctiveness
    // In this case, treat as creation scenario since we're effectively creating new habitat
    const baselineDistinctiveness = getDistinctivenessScore(baselineHabitat);
    const targetDistinctiveness = getDistinctivenessScore(habitat);

    if (baselineDistinctiveness === 0 && targetDistinctiveness > 0) {
      // Treat as creation scenario for zero distinctiveness baseline habitats
      // Use the target habitat in the enhancement table to locate the TtoT
      let timeToTarget;

      // Look up the target habitat in the enhancement table to get time-to-target
      const broadHabitat = getHabitatGroup(habitat);
      const enhancedKey = `${broadHabitat}-${habitat}`;
      const enhancedRow = enhancementMultipliers.get(enhancedKey.toLowerCase());

      if (enhancedRow) {
        // For zero distinctiveness baseline, we're effectively creating the target habitat
        // Use the "Lower Distinctiveness Habitat - Poor to Good" columns based on target habitat condition
        const columnName = `lower distinctiveness habitat - ${condition?.toLowerCase()}`;
        const timeToTargetValue = enhancedRow[columnName];
        timeToTarget = tryConvertToNumber(timeToTargetValue);
      } else {
        // Fallback to baseline habitat data if enhancement table doesn't have the habitat
        timeToTarget = getTimeToTarget(habitat, condition);
      }

      if (timeToTarget == null || timeToTarget == 'Error' || timeToTarget == 'Not Possible') {
        return { HUs: 0 };
      }

      const HUData = calculateCreationHU(size, habitat, condition, timeToTarget, timeToTargetOffset, strategicSignificanceScore, spatialRisk);
      HUData.baselineHUs = 0; // No meaningful baseline for zero distinctiveness habitat
      return HUData;
    }
    ...
    //        (Enhanced Size × Baseline Distinctiveness × Effective Baseline Condition)) ×
    //       (Temporal Risk Multiplier × Difficulty Factor) +
    //       (Enhanced Size × Baseline Distinctiveness × Effective Baseline Condition)) x
    ...
    // Use the lower of baseline or target condition score for baseline quality calculation
    const baselineConditionScore = getConditionScore(baselineCondition);
    const targetConditionScore = getConditionScore(condition);
    const effectiveBaselineConditionScore = Math.min(baselineConditionScore, targetConditionScore);

    const baselineQuality = size * getDistinctivenessScore(baselineHabitat) * effectiveBaselineConditionScore;
...
// Correct known BGS Register data errors before further processing.
// Call AFTER processHabitatSubTypes has run on areas (so broadHabitat is set).
const correctHabitatMisclassifications = (habitatGroups) => {
  // Fix typo: 'rivers and steams' → 'rivers and streams'
  for (const unit of ['areas', 'hedgerows', 'watercourses']) {
    (habitatGroups[unit] || []).forEach(h => {
      if (h.type) h.type = h.type.replace(/rivers and steams/gi, 'rivers and streams');
    });
  }

  // Move hedgerow habitats that the BGS Register incorrectly placed in the area module
  if (habitatGroups.areas) {
    const isHedgerow = h => h.broadHabitat === 'Hedgerows' || /hedgerow/i.test(h.type);
    const misplaced = habitatGroups.areas.filter(isHedgerow);
    if (misplaced.length > 0) {
      misplaced.forEach(h => { h.module = 'Hedgerow'; });
      habitatGroups.hedgerows = [...(habitatGroups.hedgerows || []), ...misplaced];
      habitatGroups.areas = habitatGroups.areas.filter(h => !isHedgerow(h));
    }
  }
};
...
export const processHabitatSubTypes = (habitats) => {
  habitats.forEach(habitat => {
    ...
    if (typeParts.length > 1) {
      habitat.broadHabitat = typeParts[0].trim();
      habitat.type = typeParts[1].trim();
    } else {
      habitat.broadHabitat = habitat.module || null;
    }
  });
}
...
/**
 * Get all habitats with their original case as they appear in the CSV file
 * @returns {string[]} Array of habitat names with original case
 */
export function getAllHabitatsWithOriginalCase() {
  return Array.from(originalHabitatNames.values()).sort();
}

/**
 * Get the original habitat names map for client-side use
 * @returns {Map} Map of lowercase habitat names to original case
 */
export function getOriginalHabitatNamesMap() {
  return originalHabitatNames;
}

/**
 * Get the distinctiveness scores map for client-side use
 * @returns {Map} Map of lowercase habitat names to distinctiveness scores
 */
export function getDistinctivenessScoresMap() {
  const scoresMap = new Map();
  habitatMap.forEach((data, key) => {
    scoresMap.set(key, data.score);
  });
  return scoresMap;
}

/**
 * Get the set of habitats where the only valid baseline condition is 'Condition Assessment N/A'.
 * These are habitats that have a numeric value in the 
 * 'Lower Distinctiveness Habitat - Condition Assessment N/A' column of the enhancement
 * multipliers CSV (i.e. the value is not 'Not Possible').
 * @returns {Set<string>} Set of lowercase habitat names
 */
export function getConditionAssessmentNAHabitats() {
  const result = new Set();
  enhancementMultipliers.forEach((row) => {
    const value = row['lower distinctiveness habitat - condition assessment n/a'];
    if (value && value.trim() !== 'Not Possible' && !isNaN(Number(value.trim()))) {
      const habitat = row['habitat']?.trim();
      if (habitat) {
        result.add(habitat.toLowerCase());
      }
    }
  });
  return result;
...
export function getAllConditions() {
  return Object.keys(conditionScores).map(cond => cond.replace(/\b[a-z]/g, l => l.toUpperCase()));
}
...
/**
 * Get the original case habitat name as it appears in the CSV file
 * @param {string} habitat The habitat type (case-insensitive)
 * @returns {string} The original case habitat name from CSV, or the input if not found
 */
export function getOriginalCaseHabitatName(habitat) {
  if (typeof habitat !== 'string') {
    return habitat;
  }
  return originalHabitatNames.get(habitat.toLowerCase()) || habitat;
}
...
const isIndividualTree = (habitat) => 
  habitat._isIndividualTree === true || getHabitatGroup(habitat.type) == 'Individual trees';
...
export const preProcessSiteHabitatData = (site) => {
  ...
  if (site.habitats) {
    if (site.habitats.areas) {
      // Mark individual trees BEFORE stripping type prefixes, so habitats whose
      // stripped type isn't in BNG-Habitat-Data.csv are still identified correctly
      site.habitats.areas.forEach(h => {
        if (h.type?.toLowerCase().includes('individual tree')) {
          h._isIndividualTree = true;
        }
      });
      // areas first need their sub-type processed out
      processHabitatSubTypes(site.habitats.areas)
      correctHabitatMisclassifications(site.habitats);
  ...
  if (site.improvements) {
    if (site.improvements?.areas) {
      // Mark individual trees before stripping type prefixes
      site.improvements.areas.forEach(h => {
        if (h.type?.toLowerCase().includes('individual tree')) {
          h._isIndividualTree = true;
        }
      });
      ...
      correctHabitatMisclassifications(site.improvements);
...
export const processSiteHabitatData = (site, habData) => {
  ...
  for (const unit of HABITAT_UNIT_TYPES) {
    ...
    site.improvements[unit].forEach(habitat => {
      ...
      // Reset created/enhanced HUs — will be re-accumulated below once subRow HUs are known
      habitat.createdHUs = 0;
      habitat.enhancedHUs = 0;
      habitat.subRows.forEach(parcel => {
        ...
        // Re-accumulate created/enhanced HUs now that subRow HUs are computed
        if (parcel.interventionType === 'Creation') {
          habitat.createdHUs += parcel.HUs;
        } else if (parcel.interventionType === 'Enhanced') {
          habitat.enhancedHUs += parcel.HUs;
        }
      });
    })
  }
  ...
  if (site.allocations) {
    site.allocations.forEach(alloc => {
      ...
      correctHabitatMisclassifications(alloc.habitats);
      ...
      // Compute total allocated area per unit type within this allocation so we can
      // distribute the allocation's official HUs proportionally by habitat area.
      const totalAllocSizeByUnit = {
        areas: (alloc.habitats.areas || []).reduce((acc, h) => acc + h.size, 0),
        hedgerows: (alloc.habitats.hedgerows || []).reduce((acc, h) => acc + h.size, 0),
        watercourses: (alloc.habitats.watercourses || []).reduce((acc, h) => acc + h.size, 0),
        trees: (alloc.habitats.trees || []).reduce((acc, h) => acc + h.size, 0),
      };
      // Official HUs per unit type from the BGS API (authoritative metric values)
      const officialUnitsByUnit = {
        areas: alloc.areaUnits || 0,
        hedgerows: alloc.hedgerowUnits || 0,
        watercourses: alloc.watercoursesUnits || 0,
        trees: 0, // no separate tree units field in the allocation API
      };

      allHabitats.forEach(habitat => {
        habitat.distinctiveness = getHabitatDistinctiveness(habitat.type);
        const unit = habitat.module == 'Area' ? 'areas' : habitat.module == 'Hedgerow' ? 'hedgerows' : habitat.module == 'Tree' ? 'trees' : 'watercourses';
        const improvements = site.improvements[unit];
        ...
        if (improvementHab) {
          ...
          // Distribute this allocation's official HUs proportionally by habitat area so
          // that sum(improvementHab.allocatedHUs) == sum(a.areaUnits / hedgerowUnits / etc.)
          // and therefore stays consistent with the Habitat Summary table.
          const totalSizeForUnit = totalAllocSizeByUnit[unit] || 0;
          const officialUnits = officialUnitsByUnit[unit] || 0;
          if (totalSizeForUnit > 0 && officialUnits > 0) {
            improvementHab.allocatedHUs = (improvementHab.allocatedHUs || 0) + officialUnits * (habitat.size / totalSizeForUnit);
          }
        }
      });
    });

    // After all allocations are processed, set the area-fraction (% Allocated) and
    // handle trees (which have no official tree units in the API).
    for (const unit of HABITAT_UNIT_TYPES) {
      site.improvements[unit].forEach(h => {
        h.allocated = (h.allocatedSize && h.allocatedSize > 0 && h.size > 0) ? h.allocatedSize / h.size : 0;
        if (unit === 'trees') {
          // Trees have no official allocation HUs in the API; use proportional HU estimate.
          h.allocatedHUs = h.allocated > 0 ? h.HUs * h.allocated : 0;
        } else if (!h.allocatedHUs) {
          h.allocatedHUs = 0;
        }
      });
    }
...
export const collateHabitats = (habitats, isImprovement) => {
  ...
  // Object.create(null) avoids prototype-chain collisions for habitat-type keys
  const collated = habitats.reduce((acc, habitat) => {
    ...
    if (!acc[key]) {
      ...
      if (isImprovement) {
        ...
        acc[key].createdSize = 0;
        acc[key].enhancedSize = 0;
        acc[key].createdHUs = 0;
        acc[key].enhancedHUs = 0;
        acc[key].createdParcels = 0;
        acc[key].enhancedParcels = 0;
        acc[key].createdSites = [];
        acc[key].enhancedSites = [];
      }
    }
    ...
    acc[key].HUs += habitat.HUs;
      ...
      if (habitat.interventionType === 'Creation') {
        // Case 1: raw parcel (site details page) — interventionType set directly on parcel
        acc[key].createdSize += habitat.size || 0;
        acc[key].createdHUs += habitat.HUs || 0;
        acc[key].createdParcels += habitat.parcels || 1;
      } else if (habitat.interventionType === 'Enhanced') {
        acc[key].enhancedSize += habitat.size || 0;
        acc[key].enhancedHUs += habitat.HUs || 0;
        acc[key].enhancedParcels += habitat.parcels || 1;
      } else if (habitat.subRows) {
        // Case 2: pre-collated DB habitat (Habitat Finder page) — interventionType lives on subRows
        const subRowsArr = Array.isArray(habitat.subRows) ? habitat.subRows : Object.values(habitat.subRows);
        subRowsArr.forEach(sub => {
          if (sub.interventionType === 'Creation') {
            acc[key].createdSize += sub.size || 0;
            acc[key].createdHUs += sub.HUs || 0;
            acc[key].createdParcels += sub.parcels || 1;
          } else if (sub.interventionType === 'Enhanced') {
            acc[key].enhancedSize += sub.size || 0;
            acc[key].enhancedHUs += sub.HUs || 0;
            acc[key].enhancedParcels += sub.parcels || 1;
          }
        });
      }
    }
    ...
    if (habitat.site) {
      ...
      // Track sites separately by intervention type for the # Sites breakdown
      if (isImprovement) {
        const siteRef = habitat.site.r;
        if (habitat.interventionType === 'Creation') {
          // Case 1: raw parcel — interventionType set directly
          if (!acc[key].createdSites.find(s => s.r == siteRef)) {
            acc[key].createdSites.push({ r: siteRef });
          }
        } else if (habitat.interventionType === 'Enhanced') {
          if (!acc[key].enhancedSites.find(s => s.r == siteRef)) {
            acc[key].enhancedSites.push({ r: siteRef });
          }
        } else if (habitat.subRows) {
          // Case 2: pre-collated habitat — check subRows for intervention types
          const subRowsArr = Array.isArray(habitat.subRows) ? habitat.subRows : Object.values(habitat.subRows);
          const hasCreation = subRowsArr.some(sub => sub.interventionType === 'Creation');
          const hasEnhanced = subRowsArr.some(sub => sub.interventionType === 'Enhanced');
          if (hasCreation && !acc[key].createdSites.find(s => s.r == siteRef)) {
            acc[key].createdSites.push({ r: siteRef });
          }
          if (hasEnhanced && !acc[key].enhancedSites.find(s => s.r == siteRef)) {
            acc[key].enhancedSites.push({ r: siteRef });
          }
        }
      }
