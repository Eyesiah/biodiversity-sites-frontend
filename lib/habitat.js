...
export function calcEnhancementTemporal(improvementHab, baselineHabitats, module) {
  ...
  const improvementDistinctiveness = getHabitatDistinctiveness(improvementHab.type);
  ...
  // First priority: same habitat type, condition improvement
  ...
  if (sameHabitatBaselines.length > 0) {
    ...
    bestMatch = sameHabitatBaselines[0]; // Could be enhanced, just use first for now
  }

  // Second priority: different habitat types, distinctiveness improvement
  if (!bestMatch) {
    const distinctivenessImprovementBaselines = matchingBaselineHabitats.filter(hab =>
      hab.type !== improvementHab.type &&
      getDistinctivenessScore(hab.type) < getDistinctivenessScore(improvementHab.type)
    );

    if (distinctivenessImprovementBaselines.length > 0) {
      improvementType = 'distinctiveness';
      bestMatch = distinctivenessImprovementBaselines[0];
    }
  }
  ...
  if (moduleType === 'watercourses') {
    if (improvementType === 'distinctiveness') {
      ...
      return {
        timetotarget: '10',
        temporalMultiplier: calcTemporalRisk(improvementHab.type, improvementHab.condition)
      };
  ...
  // Lookup enhancement time from CSV
...
function calculateEnhancedHU(size, habitat, condition, temporalMultiplier, difficultyMultiplier, baselineDistinctiveness, baselineCondition, strategicSignificance = 1.0) {
  ...
  if (habitat.toLowerCase().includes('modified grassland') ||
  ...
  improvements.forEach(improvement => {
    if (improvement.interventionType && improvement.interventionType.toLowerCase() === 'enhanced') {
      ...
      if (enhancementResult) {
        improvement.enhancementTimeToTarget = enhancementResult.timetotarget;
        improvement.enhancementTemporalMultiplier = enhancementResult.temporalMultiplier;
...
export const processSiteHabitatData = (site) => {
  ...
  if (site.improvements) {
    ...
    if (site.improvements?.areas) {
      ...
      // Process enhancement temporal calculations
...
export const collateHabitats = (habitats, isImprovement) => {
  ...
  const collated = habitats.reduce((acc, habitat) => {
    ...
    if (!acc[key].subRows[subKey]) {
      const isEnhancement = isImprovement && habitat.interventionType && habitat.interventionType.toLowerCase() === 'enhanced';
      acc[key].subRows[subKey] = {
        condition: habitat.condition,
        interventionType: habitat.interventionType,
        type: habitat.type,
        timetotarget: isImprovement ? (isEnhancement ? habitat.enhancementTimeToTarget : getTimeToTarget(habitat.type, habitat.condition)) : null,
        temporalMultiplier: isImprovement ? (isEnhancement ? habitat.enhancementTemporalMultiplier : calcTemporalRisk(habitat.type, habitat.condition)) : null,
        difficultyFactor: isImprovement ? calcDifficultyFactor(habitat.type) : null,
        spatialRisk: isImprovement ? 1.0 : null, // Default to 1.0 for now
        parcels: 0,
        area: 0,
        HUs: 0,
      };
