...
function loadLPAData() {
  ...
  lpas.forEach(lpa => lpaMap.set(lpa.name, lpa));
...
function loadNCAData() {
  ...
  rawNcas.forEach(nca => ncaMap.set(nca.name, nca));
...
export const calcSpatialRiskCategory = (alloc, site) => { 
  ...
  // if LPA or NCA match, alloc is Within
  if (site.lpaName && alloc.localPlanningAuthority) {
    if (site.lpaName == alloc.localPlanningAuthority) {
      return 'Within';
    }
  }
  if (site.ncaName && alloc.nca) {
    if (site.ncaName == alloc.nca) {
      return 'Within';
    }
  }

  // if LPA or NCA are adjacent, alloc is Neighbouring
  if (site.lpaName && alloc.localPlanningAuthority) {
    const adjacentLPAs = lpaData.get(site.lpaName)?.adjacents || [];
    if (adjacentLPAs.find(alpa => alloc.localPlanningAuthority == alpa.name)) {
      return 'Neighbouring';
    }
  }
  if (site.ncaName && alloc.nca) {
    const adjacentNCAs = ncaData.get(site.ncaName)?.adjacents || [];
    if (adjacentNCAs.find(anca => alloc.nca == anca.name)) {
      return 'Neighbouring';
    }
  }

  // otherwise, alloc is Outside
  return 'Outside';
}
...
export const collateHabitats = (habitats, isImprovement) => {
  ...
  Object.values(collated).forEach(h =>{
    // convert allocations to a percentage
    if (h.allocatedArea && h.allocatedArea > 0 && h.area > 0) {
      h.allocated = (h.allocatedArea / h.area).toFixed(4);
    } else {
      h.allocated = 0;
    }
    delete h.allocatedArea;
  });
