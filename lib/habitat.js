...
export function getEffectiveTimeToTarget(timeToTarget, timeToTargetOffset = 0) {
  ...
  // Debug logging to verify offset values
  console.log(`[getEffectiveTimeToTarget] Input: timeToTarget="${timeToTarget}", timeToTargetOffset=${timeToTargetOffset} (type: ${typeof timeToTargetOffset})`);
  ...
  if (timeToTargetOffset !== 0) {
    if (typeof timeToTarget === 'string' && !isNaN(Number(timeToTarget))) {
      ...
      console.log(`[getEffectiveTimeToTarget] Base years: ${baseYears}, Offset: ${timeToTargetOffset}, Net: ${effectiveTimeToTarget}`);
    } else if (timeToTarget === "30+") {
      // For 30+, apply offset but don't go below 0
      effectiveTimeToTarget = Math.max(0, 30 + timeToTargetOffset);
      console.log(`[getEffectiveTimeToTarget] 30+ case: Base: 30, Offset: ${timeToTargetOffset}, Net: ${effectiveTimeToTarget}`);
      ...
      console.log(`[getEffectiveTimeToTarget] Number case: Base: ${timeToTarget}, Offset: ${timeToTargetOffset}, Net: ${effectiveTimeToTarget}`);
    } else {
      // For other cases (Not Possible, null, etc.), don't apply offset
      console.log(`[getEffectiveTimeToTarget] No offset applied for non-numeric timeToTarget: ${timeToTarget}`);
    }
  }

  console.log(`[getEffectiveTimeToTarget] Final result: ${effectiveTimeToTarget}`);
...
function calcTemporalRisk(habitat, condition, timeToTarget, timeToTargetOffset = 0) {
  ...
  // Debug logging to verify temporal multiplier calculation
  console.log(`[Temporal Risk Debug] Habitat: ${habitat}, Condition: ${condition}`);
  console.log(`[Temporal Risk Debug] Base time to target: ${timeToTarget}`);
  console.log(`[Temporal Risk Debug] Time to target offset: ${timeToTargetOffset}`);
  console.log(`[Temporal Risk Debug] Effective time to target (net): ${effectiveTimeToTarget}`);

  if (effectiveTimeToTarget == "Not Possible") {
    const multiplier = temporalMultipliers.get("Not Possible") || 0;
    console.log(`[Temporal Risk Debug] Using 'Not Possible' multiplier: ${multiplier}`);
    return multiplier;
  }
  else if (effectiveTimeToTarget == null) {
    if (condition == "Unable to Map") {
      console.log(`[Temporal Risk Debug] Condition 'Unable to Map' - using multiplier: 1`);
      return 1;
    }
    else {
    ...
    console.log(`[Temporal Risk Debug] Using '30+' multiplier: ${multiplier}`);
    return multiplier;
  }
  else {
    ...
    if (!isNaN(timeToTargetYears)) {
      ...
      if (multiplier !== null && multiplier !== undefined) {
        console.log(`[Temporal Risk Debug] Found exact multiplier for ${timeToTargetYears} years (${exactYearKey}): ${multiplier}`);
        return multiplier;
      }
      else {
        ...
        if (timeToTargetYears > 31) {
          ...
          if (fallbackMultiplier !== null && fallbackMultiplier !== undefined) {
            console.log(`[Temporal Risk Debug] Using '31+' multiplier for ${timeToTargetYears} years: ${fallbackMultiplier}`);
        ...
        console.log(`[Temporal Risk Debug] No exact match for ${timeToTargetYears} years, using '30+' fallback: ${fallbackMultiplier}`);
