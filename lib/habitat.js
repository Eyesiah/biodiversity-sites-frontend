...
/**
 * Get the baseline condition score for enhancement calculations.
 * For N/A conditions, use a score of 0.5 to allow meaningful enhancement calculations.
 * For habitats with zero distinctiveness, return 0 to treat as creation.
 * @param {string} condition The condition of the habitat parcel.
 * @param {string} habitat The habitat type.
 * @returns {number} The baseline condition score.
 */
export function getBaselineConditionScore(condition, habitat = '') {
  if (typeof condition !== 'string') {
    console.error('getBaselineConditionScore: "condition" must be a string.');
    return 0.5;
  }

  const conditionLower = condition.toLowerCase().trim();
  
  // For habitats with zero distinctiveness, treat as creation (score 0)
  if (habitat && getDistinctivenessScore(habitat) === 0) {
    return 0;
  }
  
  // For N/A conditions in habitats with distinctiveness > 0, use 0.5 to allow meaningful enhancement calculations
  if (conditionLower === 'condition assessment n/a' || conditionLower === 'n/a - other') {
    return 0.5;
  }

  return conditionScores[conditionLower] || 0;
}
...
export function calculateImprovementHU(size, habitat, condition, improvementType, baselineHabitat = null, baselineCondition = null, timeToTargetOffset = 0, strategicSignificanceScore = 1.0, spatialRisk = 1.0) {
  ...
  if (improvementType.toLowerCase() == 'creation') {
    ...
    const baselineQuality = size * getDistinctivenessScore(baselineHabitat) * getBaselineConditionScore(baselineCondition);
