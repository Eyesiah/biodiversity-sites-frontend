...
export const calcSpatialRiskCategory = (alloc, site) => {
  ...
  if (siteNCA && allocNCA) {
    const adjacentNCASlugs = new Set(
      (ncaData.get(siteNCA)?.adjacents || []).map(a => slugify(normalizeBodyName(a.name)))
    );
    if (adjacentNCASlugs.has(allocNCA)) {
      return { cat: 'Neighbouring', from: 'NCA' };
    }
  }
