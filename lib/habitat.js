  ...
  for (const unit of HABITAT_UNIT_TYPES) {
    ...
    site.improvements[unit].forEach(habitat => {
      ...
      habitat.subRows.forEach(parcel => {
        ...
        if (parcel.interventionType.toLowerCase() == 'enhanced') {
          ...
          // for creations, we can just do a single calcuation as no connection to base parcel(s) necessary
          Object.assign(parcel, calculateImprovementHU(parcel.size, habitat.type, parcel.condition, parcel.interventionType));
        }
        habitat.HUs += parcel.HUs;
      });
    })
