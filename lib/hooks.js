...
export const useSearchAndSort = (initialItems, filterPredicate, initialSortConfig, debounceMs = 300) => {
    ...
    // Detect when filter is cleared (transition from non-empty to empty)
    const isFilterCleared = useMemo(() => {
      const wasPreviouslyFiltered = prevSearchTermRef.current !== '';
      const isCurrentlyCleared = debouncedSearchTerm === '';
      const isClearedNow = wasPreviouslyFiltered && isCurrentlyCleared;
