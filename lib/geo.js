...
export async function getCoordinatesForAddress(address, lpaName = null) {
  ...
  // Combine address and LPA for a more specific query; append UK also
  const fullAddress = lpaName ? `${address}, ${lpaName}, UK` : address;
  ...
  try {
    ...
    if (data.status === 'OK' && data.results.length > 0) {
      ...
      // filter UK locations only
      const validLocations = data.results.filter(r => r.address_components.find(c => c.types.includes('country'))?.short_name == "GB");      
...
export async function getCoordinatesForLPA(lpaName) {
  ...
  const coords = await getCoordinatesForAddress(lpaName);
...
export const fetchAllocSpatialData = async (alloc, site) => {
  ...
  if (alloc.coords) {
    ...
    if (alloc.lsoa == null) {
      ...
      try {
        const lsoaRes = await fetch(lsoaUrl, {next: { revalidate: 3600 } });
