...
export const fetchSiteSpatialData = async (site) => {
  ...
  try {
    const [lsoaRes, lnrsRes, ncaRes, lpaRes] = await Promise.all([
      fetch(lsoaUrl, {cache: 'force-cache', next: { revalidate: 3600 } }),
      fetch(lnrsUrl, {cache: 'force-cache', next: { revalidate: 3600 } }),
      fetch(ncaUrl, {cache: 'force-cache', next: { revalidate: 3600 } }),
      fetch(lpaUrl, {cache: 'force-cache', next: { revalidate: 3600 } }),
    ]);
