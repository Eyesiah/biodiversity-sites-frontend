...
import { getLNRSLink } from '@/lib/LNRS'
...
import { fixLpaApostrophe } from '@/lib/format';
...
export const getCoordinatesForAddress = unstable_cache(
  async (address) => {
...
const querySiteGeoCache = unstable_cache(
  async (id, loc) => {
    const cacheKey = { id: id, loc: loc };
    const client = await clientPromise;
    const db = client.db(MONGODB_DATABASE_NAME);
...
export const fetchSiteSpatialData = async (site) => {
  ...
  if (clientPromise) {
    try {
      ...
      const cachedResult = await querySiteGeoCache(site.referenceNumber, site.gridReference);
      if (cachedResult) {
        site.lnrsName = cachedResult.lnrsName;
        site.lnrsUrl = getLNRSLink(site.lnrsName);
        site.ncaName = cachedResult.ncaName;
        site.lpaName = fixLpaApostrophe(cachedResult.lpaName);
  ...
  try {
    const [lsoaRes, lnrsRes, ncaRes, lpaRes] = await Promise.all([
      fetch(lsoaUrl, { next: { revalidate: 21600 }, signal: AbortSignal.timeout(10000) }),
      fetch(lnrsUrl, { next: { revalidate: 21600 }, signal: AbortSignal.timeout(10000) }),
      fetch(ncaUrl, { next: { revalidate: 21600 }, signal: AbortSignal.timeout(10000) }),
      fetch(lpaUrl, { next: { revalidate: 21600 }, signal: AbortSignal.timeout(10000) }),
    ]);
    ...
    site.lpaName = fixLpaApostrophe(lpaData.features?.[0]?.attributes?.LPA23NM.trim() || 'N/A');
    ...
    if (clientPromise) {
      try {
        const cacheKey = { id: site.referenceNumber, loc: site.gridReference };
        const client = await clientPromise;
        const db = client.db(MONGODB_DATABASE_NAME);
        const cache = db.collection('siteGeoCache');
        await cache.updateOne(
...
const queryAllocGeoCache = unstable_cache(
  async (allocID) => {
    const cacheKey = { allocID: allocID };
    const client = await clientPromise;
    const db = client.db(MONGODB_DATABASE_NAME);
...
export const fetchAllocSpatialData = async (alloc, site) => {
  ...
  if (alloc.coords) {
    ...
    if (clientPromise) {
      try {
        const cachedResult = await queryAllocGeoCache(alloc.developerReference ?? alloc.planningReference);
        if (cachedResult) {
          const lsoaName = cachedResult.lsoaName;
          const lsoaInfo = getLSOAInfo(lsoaName);
          alloc.lsoa = {
    ...
    // query arcgis if not in our cache, or if lnrs is missing from an older cache entry
    if (alloc.lsoa == null || alloc.lnrs == null) {
      ...
      try {
        const [lsoaRes, ncaRes, lnrsRes] = await Promise.all([
          fetch(lsoaUrl, { next: { revalidate: 21600 }, signal: AbortSignal.timeout(10000) }),
          fetch(ncaUrl, { next: { revalidate: 21600 }, signal: AbortSignal.timeout(10000) }),
          fetch(lnrsUrl, { next: { revalidate: 21600 }, signal: AbortSignal.timeout(10000) }),
        ]);

        const [lsoaData, ncaData, lnrsData] = await Promise.all([
          lsoaRes.json(),
          ncaRes.json(),
          lnrsRes.json(),
        ]);
        ...
        alloc.lnrs = lnrsData.features?.[0]?.attributes?.Name.trim() || 'N/A';
