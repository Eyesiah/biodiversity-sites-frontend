...
async function fetchAllocationWithSiteData(lpa, planningRef) {
  ...
  const sitesWithSpatialData = await Promise.all(
    Array.from(siteRefNumbers).map(refNo => 
      fetchSite(refNo, true, true, true)
    )
  );
...
export { fetchAllSites, queryBGSAPI, fetchSite, fetchAllRefNos, fetchAllAllocationsForStaticParams, transformAllocations, fetchAllocationWithSiteData };
