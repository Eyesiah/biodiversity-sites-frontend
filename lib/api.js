...
import { ARCGIS_LSOA_URL, ARCGIS_LNRS_URL } from '@/config'
...
const fetchSpatialData = async (site) => {
  ...
  const lsoaUrl = `${ARCGIS_LSOA_URL}?geometry=${site.longitude},${site.latitude}&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelIntersects&outFields=LSOA11NM&returnGeometry=false&f=json`;
  const lnrsUrl = `${ARCGIS_LNRS_URL}?geometry=${site.longitude},${site.latitude}&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelIntersects&outFields=Name&returnGeometry=false&f=json`;
