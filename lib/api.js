...
async function fetchAllSites(withSiteSpatialData=false, withAllocSpatialData=false) {
  let allSites = [];
  const batchSize = 20;
  ...
  for (let i = 0; i < siteList.length; i += batchSize) {
    ...
    const batchPromises = batch.map(site => fetchSite(site.referenceNumber, withSiteSpatialData, withAllocSpatialData));
...
async function fetchSite(referenceNumber, withSiteSpatialData=false, withAllocSpatialData=false) {
