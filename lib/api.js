...
import { preProcessSiteHabitatData, processSiteHabitatData, calcSpatialRiskCategory, getHabitatGroup } from '@/lib/habitat';
...
import { slugify, fixLpaApostrophe } from '@/lib/format';
...
const getNameRecord = unstable_cache(
  async (refNo) => {
    const client = await clientPromise;
    const db = client.db(MONGODB_DATABASE_NAME);
    const cache = db.collection('siteName');
    const cacheKey = { _id: refNo };
    return await cache.findOne(cacheKey);
  }, [], { revalidate: 86400 });

const getPublishedDate = unstable_cache(
  async (refNo) => {
    const client = await clientPromise;
    const db = client.db(MONGODB_DATABASE_NAME);
    const record = await db.collection('sites').findOne({ id: refNo }, { projection: { publishedDate: 1 } });
    if (!record) return { publishedDate: null, isKnown: false };
    return { publishedDate: record.publishedDate ?? null, isKnown: true };
  }, [], { revalidate: 86400 });

const getEarliestStatisticsDate = unstable_cache(
  async () => {
    const client = await clientPromise;
    const db = client.db(MONGODB_DATABASE_NAME);
    const record = await db.collection('statistics').findOne({}, { sort: { timestamp: 1 }, projection: { timestamp: 1 } });
...
async function getSiteName(site) {
  ...
  if (clientPromise) {
    try {
      const [nameRecord, { publishedDate, isKnown }] = await Promise.all([
        getNameRecord(site.referenceNumber),
        getPublishedDate(site.referenceNumber),
      ]);
      if (nameRecord) {
        site.name = nameRecord.name;
        site.bgsReference = nameRecord.bgsReference || null;
        site.bgsReferenceUrl = nameRecord.bgsReferenceUrl || null;
        site.bgsWebsite = nameRecord.bgsWebsite || null;
        site.miscUrls = nameRecord.miscUrls || null;
        // Use our blob-hosted boundary map as fallback if BGS register has none
        if (!site.landBoundary && nameRecord.boundaryMapUrl) {
          site.landBoundary = nameRecord.boundaryMapUrl;
        }
        if (nameRecord.metricFileUrl) {
          site.metricFileUrl = nameRecord.metricFileUrl;
          site.metricFileName = nameRecord.metricFileName || null;
        }
      }
      if (publishedDate) {
        site.publishedDate = publishedDate;
      } else if (isKnown) {
        const earliestDate = await getEarliestStatisticsDate();
        site.publishedDate = earliestDate;
        site.publishedDatePreTracking = true;
      }
    } catch (e) {
...
const getAllSiteNames = unstable_cache(
  async (referenceNumbers) => {
    ...
    try {
      ...
      const records = await cache.find(
        ...
        { projection: { name: 1, bgsWebsite: 1, bgsReferenceUrl: 1, miscUrls: 1 } }
      ).toArray();
      const namesMap = {};
...
const queryBGSAPI = unstable_cache(
  async (endpoint, verb = 'POST') => {

    // check our mongodb cache  
    const client = await clientPromise;
    const db = client.db(MONGODB_DATABASE_NAME);
    const cache = db.collection('bgs-register-cache');
    const cacheKey = { endpoint: endpoint, verb: verb };
    const cacheRecord = await cache.findOne(cacheKey);
    if (cacheRecord && cacheRecord.data) {
      return cacheRecord.data;
    }

    // fallback to querying the API directly if its not in our cache
...
async function fetchAllSites(withSiteSpatialData = false, withAllocSpatialData = false, withNames = false) {
  ...
  if (withNames) {
    ...
    allSites.forEach(site => {
      const record = namesMap[site.referenceNumber];
      if (record) {
        site.name = record.name;
        site.bgsWebsite = record.bgsWebsite;
        site.bgsReferenceUrl = record.bgsReferenceUrl;
        site.miscUrls = record.miscUrls;
      }
    });
  }
...
async function fetchSite(referenceNumber, withSiteSpatialData = false, withAllocSpatialData = false, withNames = false, withSankey = false) {
  ...
  // The BGS register API returns LPA names with a mangled apostrophe for "King's Lynn and West
  // Norfolk" (a UTF-8/Latin-1 mojibake artifact) - fix at the source so every downstream
  // consumer of alloc.localPlanningAuthority gets the correct name.
  (site.allocations || []).forEach(alloc => {
    if (alloc.localPlanningAuthority) {
      alloc.localPlanningAuthority = fixLpaApostrophe(alloc.localPlanningAuthority);
    }
  });

  if (site.gridReference) {
  ...
  return allSites.flatMap(site => {
    ...
    return site.allocations.map(alloc => {
      return {
        ...
        siteLpa: site.lpaName,
        lnrs: site.lnrsName,
        allocLnrs: alloc.lnrs,
        nca: site.ncaName,
        allocNca: alloc.nca,
        ...
        siteName: site.name,
        rb: site.responsibleBodies,
        d: alloc.distance,
        sr: alloc.sr,
        srCat: alloc.sr?.cat || 'Unknown',
        ...
        habitats: alloc.habitats
          ? Object.fromEntries(
            HABITAT_UNIT_TYPES.map(unit => [
              unit,
              (alloc.habitats[unit] || []).map(h => {
                // Derive broadHabitat from the habitat type.
                // Try the raw type first; if not found, strip the broad-type prefix
                // (e.g. "Grassland and Shrub Land - Good quality..." → "Good quality...")
                let broadHabitat = getHabitatGroup(h.type);
                if (broadHabitat === 'N/A' && h.type?.includes(' - ')) {
                  const stripped = h.type.split(' - ').slice(1).join(' - ').trim();
                  broadHabitat = getHabitatGroup(stripped);
                }
                return { ...h, broadHabitat };
              })
            ])
          )
          : null
...
const getAllocMatchCache = unstable_cache(
  async (lpa, planningRef) => {
...
async function fetchAllocationWithSiteData(lpa, planningRef) {
  ...
  const summary = {
    ...
    latitude: firstAlloc.coords?.latitude ?? null,
    longitude: firstAlloc.coords?.longitude ?? null,
  };
...
const getUnslugifiedValues = unstable_cache(
  async (slugifiedLpa, slugifiedRef) => {
...
export { fetchAllSites, fetchSite, fetchAllRefNos, fetchAllAllocationsForStaticParams, transformAllocations, fetchAllocationWithSiteData, cacheSlugMappingsBulk, getUnslugifiedValues };
