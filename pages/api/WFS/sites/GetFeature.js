...
export default async function handler(req, res) {
  ...
  const data = allSites.map(s => ({
    ...
    'responsible-body': s.responsibleBodies,
    'latitude': s.latitude,
  ...
  if (outputFormat == null || outputFormat.includes('xml')) {
    res.setHeader('Content-Type', 'application/xml');

    const protocol = req.headers['x-forwarded-proto'] || 'http';
    const host = req.headers.host;
    const absoluteSchemaUrl = `${protocol}://${host}/api/WFS/sites/DescribeFeatureType`;
  ...
  } else if (outputFormat.includes('json')) {
