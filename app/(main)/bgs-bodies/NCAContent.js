      ...
      <SearchableTableLayout
        ...
        tabs={[
          ...
          },
          {
            title: "Chart",
            content: ({ sortedItems }) => {
              const filteredNcaNames = new Set(sortedItems.map(nca => nca.name));
              const filteredSites = sites.filter(site => {
                const normalizedSiteName = normalizeBodyName(site.ncaName);
                return Array.from(filteredNcaNames).some(ncaName => 
                  slugify(normalizeBodyName(ncaName)) === slugify(normalizedSiteName)
                );
              });
              return <NCAMetricsChart sites={filteredSites} />;
            }
          }
        ]}
      />
